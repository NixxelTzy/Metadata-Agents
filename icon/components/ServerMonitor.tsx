...
import { Radio, Activity, Triangle, Shield, BarChart3, AlertTriangle, RefreshCw, Clock, Lock, Check, Copy } from "lucide-react";
...
function GlassOverlay({
  ...
  attack: MonitorData["security"]["recentAttacks"][number];
  onClose: () => void;
}) {
  ...
  const expiry = attack.timestamp + ttlMs;
  ...
  const pctRemaining = Math.min(100, Math.max(0, (remainingSec / 86400) * 100));

  const copyForensic = () => {
    const text = `BLOCKED FORENSIC DETAIL:\nEndpoint: ${attack.method} ${attack.endpoint}\nIP: ${attack.ip}\nReason: ${attack.reason || "blocked_by_missing_email"}\nTimestamp: ${new Date(attack.timestamp).toISOString()}\nTTL Remaining: ${hours}h ${minutes}m ${seconds}s`;
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 17, fontWeight: 900, color: "#f87171", letterSpacing: "0.02em" }}>
                AKSES FITUR DIBLOKIR SEMENTARA
              </div>
              <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>
                Marker: <code style={{ color: "#fca5a5", background: "rgba(239,68,68,0.15)", padding: "1px 5px", borderRadius: 4 }}>blocked_by_missing_email</code>
              </div>
            </div>
          </div>
          <button
        ...
        >
          <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
            <Clock size={13} color="#f87171" />
            Sisa Waktu Blokir 24 Jam
          </div>
          <div
            style={{
              display: "flex",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#6b7280", marginTop: 8, padding: "0 28px" }}>
            <span>JAM</span>
            <span>MENIT</span>
            <span>DETIK</span>
          </div>
          ...
          <div style={{ marginTop: 12, height: 4, background: "rgba(255, 255, 255, 0.08)", borderRadius: 2, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${pctRemaining}%`, background: "linear-gradient(90deg, #ef4444, #f87171)", transition: "width 1s linear" }} />
          </div>
        </div>

        {/* Detail info */}
        <div style={{ marginTop: 14, background: "rgba(255, 255, 255, 0.03)", border: "1px solid rgba(255, 255, 255, 0.08)", borderRadius: 12, padding: "12px 14px", fontSize: 12, display: "flex", flexDirection: "column", gap: 7 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ color: "#9ca3af" }}>Target Endpoint:</span>
            <span style={{ fontWeight: 700, color: "#e5e7eb", fontFamily: "monospace" }}>
              {attack.method} {attack.endpoint}
            </span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ color: "#9ca3af" }}>IP Terblokir:</span>
            <span style={{ fontWeight: 700, color: "#f87171", fontFamily: "monospace" }}>{attack.ip}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ color: "#9ca3af" }}>Waktu Kejadian:</span>
            <span style={{ color: "#d1d5db" }}>{new Date(attack.timestamp).toLocaleTimeString("id-ID")}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ color: "#9ca3af" }}>Alasan:</span>
            <span style={{ color: "#fca5a5", fontSize: 11, background: "rgba(239, 68, 68, 0.15)", padding: "1px 6px", borderRadius: 4, fontFamily: "monospace" }}>
              {attack.reason || "blocked_by_missing_email"}
            </span>
          </div>
        </div>
        ...
        <div style={{ marginTop: 12, fontSize: 11, color: "#9ca3af", lineHeight: 1.5, textAlign: "center" }}>
          Akses terhadap endpoint fitur utama dibatasi hanya untuk sesi pengguna dengan email terautentikasi (auth_token).
        </div>
...
export default function ServerMonitor() {
  ...
  // ── Defence/Attacks feature filter & overlay state ────────────────────────
  const [filterFeatureOnly, setFilterFeatureOnly] = useState(true);
  const [dismissedBlockedId, setDismissedBlockedId] = useState<string | null>(null);

  const isFeatureAlert = useCallback((a: MonitorData["security"]["recentAttacks"][number]) => {
    const isFeatureEndpoint = [
      "/api/research",
      "/api/generate",
      "/api/vector",
      "/api/validate/links",
      "/api/chat",
    ].some((p) => a.endpoint === p || a.endpoint?.startsWith(p));
    const hasMissingEmail =
      (a.reason ?? "").includes("blocked_by_missing_email") ||
      (a.blockedReason ?? "").includes("blocked_by_missing_email") ||
      a.signals?.some((s) => s.detail?.includes("blocked_by_missing_email"));
    return isFeatureEndpoint || hasMissingEmail;
  }, []);

  const missingEmailAttack = (data?.security?.recentAttacks ?? []).find(
    (a) =>
      (a.reason ?? "").includes("blocked_by_missing_email") ||
      (a.blockedReason ?? "").includes("blocked_by_missing_email") ||
      a.signals?.some((s) => s.detail?.includes("blocked_by_missing_email"))
  ...
  return (
    <div className="mon-panel">
      ...
      {/* ── Glassmorphism Overlay — blocked_by_missing_email ── */}
      {missingEmailAttack && dismissedBlockedId !== missingEmailAttack.requestId && (
        <GlassOverlay
          attack={missingEmailAttack}
          onClose={() => setDismissedBlockedId(missingEmailAttack.requestId)}
        />
      )}

      {/* Missing email mini-alert banner (persistent, above tabs) */}
      {missingEmailAttack && dismissedBlockedId === missingEmailAttack.requestId && (
        <div style={{ padding: "7px 12px", background: "rgba(239,68,68,0.08)", borderBottom: "1px solid rgba(239,68,68,0.2)", display: "flex", alignItems: "center", gap: 8 }}>
          <Lock size={13} color="#ef4444" />
          <span style={{ fontSize: 11, color: "#ef4444", flex: 1, fontWeight: 600 }}>
            {missingEmailAttack.ip} diblokir 24 jam ({missingEmailAttack.reason || "blocked_by_missing_email"})
          </span>
          <button
            type="button"
            onClick={() => setDismissedBlockedId(null)}
            style={{ fontSize: 10, color: "#ef4444", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 5, padding: "2px 8px", cursor: "pointer" }}
          >
            Detail
          </button>
        </div>
      ...
      <div className="mon-body">
        ...
        {tab === "attacks" && (
          <div className="mon-section">
            <div className="mon-section__title" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
              <span>
                {filterFeatureOnly ? "Feature-Usage Alerts" : "All Requests — Real-time"}
                <span className="mon-badge" style={{ marginLeft: 6 }}>
                  {filterFeatureOnly
                    ? data.security.recentAttacks.filter(isFeatureAlert).length
                    : data.security.recentAttacks.length}
                </span>
              </span>
              <button
                type="button"
                onClick={() => setFilterFeatureOnly((v) => !v)}
                style={{
                  fontSize: 10,
                  fontWeight: 700,
                  padding: "3px 10px",
                  borderRadius: 6,
                  cursor: "pointer",
                  border: `1px solid ${filterFeatureOnly ? "#ef4444" : "var(--border)"}`,
                  background: filterFeatureOnly ? "rgba(239,68,68,0.08)" : "var(--bg-secondary)",
                  color: filterFeatureOnly ? "#ef4444" : "var(--text-muted)",
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 5,
                }}
              >
                <Lock size={10} />
                {filterFeatureOnly ? "Hanya Feature-Usage" : "Semua Traffic"}
              </button>
            </div>
            {(filterFeatureOnly ? data.security.recentAttacks.filter(isFeatureAlert) : data.security.recentAttacks).length === 0
              ? <div className="mon-empty">{filterFeatureOnly ? "Tidak ada feature-usage alert terdeteksi." : "Belum ada request masuk tercatat."}</div>
              : (
                <div className="mon-attack-list">
                  {(filterFeatureOnly ? data.security.recentAttacks.filter(isFeatureAlert) : data.security.recentAttacks).map((a) => {
                    ...
                    const hasMissingEmail =
                      ...
                      a.signals?.some((s) => s.detail?.includes("blocked_by_missing_email"));
                    return (
                      <div key={a.requestId} className="mon-attack-item" style={{ borderLeft: `3px solid ${hasMissingEmail ? "#ef4444" : severityColor(a.severity)}` }}>
                        <div className="mon-attack-item__header">
                          <span className="mon-attack-item__type" style={{ color: hasMissingEmail ? "#ef4444" : severityColor(a.severity) }}>
                            ● {hasMissingEmail ? "blocked by missing email" : a.type.replace(/_/g, " ")}
                            ...
                            {hasMissingEmail && <span style={{ marginLeft: 6, fontSize: 10, background: "rgba(239,68,68,0.1)", color: "#ef4444", padding: "1px 6px", borderRadius: 3, border: "1px solid rgba(239,68,68,0.3)", fontWeight: 800, letterSpacing: "0.04em" }}>24H BAN</span>}
                          </span>
