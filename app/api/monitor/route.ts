...
function buildSnapshot() {
  const secStats = securityLogger.getStats();
  const recentAttacks = securityLogger.getRecentEvents(20).map((e) => ({
    type: e.type,
    severity: e.severity,
    ip: e.ip ?? "unknown",
    endpoint: e.endpoint ?? "/",
  ...
  return {
    ...
    security: {
      ...
      recentAttacks,
      defenceStatus: {
        attackDefence: "active",
        bypassPrevention: "active",
        fileEnumeration: "active",
        rateLimiter: "active",
        ipAnalyzer: "active",
